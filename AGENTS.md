...
| 层级 | 技术 |
|------|------|
| 后端 | Spring Boot 3.4.4 + MyBatis-Plus 3.5.5 + MySQL 8.0 |
| 前端 | Vue 3 + Vite + TypeScript + Pinia + Vue Router 4 |
| AI | Spring AI Openai Starter 1.0.0-M6 (DashScope 兼容模式 Chat) + 自研 DashScopeEmbeddingService (DashScope 原生 Embedding API, text-embedding-v3) |
...
15. **TimelineEvent.java 已添加 summary 字段**：实体类已映射 DB schema 的 summary 列，ContentIndexService 已联动
16. **ContentIndexService 联动**：作品/技能/经历 CRUD 时自动调用 indexXxx() 或 removeXxxIndex()
17. **AdminController 已拆分**：原单体 AdminController 已拆分为 controller/admin/ 下的 7 个独立 Controller（Auth/Profile/Project/Skill/Timeline/Knowledge/Message），原文件已废弃
...
### PowerShell 反引号转义陷阱（极易踩坑）

用 PowerShell 修改 Java 源码时，以下操作会把 `` `n ``、`` `t `` 等 PowerShell 转义序列嵌入 Java 源文件，导致 `java: 非法字符`：

| 禁止做法 | 正确替代 |
|----------|----------|
| `$c -replace 'old', "new`nmore"` （PowerShell 运算符会展开转义） | `$c.Replace('old', "new`nmore")` （.NET 方法不展开） |
| `$c -replace 'old', 'new``nmore'` （反引号被展开） | `$c.Replace('old', 'new` + "``n" + `more')` |
| 用 `-replace` 做纯文本替换 | 用 `.Replace()` 方法（C# 语义，反引号当普通字符） |

**铁律：修改 Java/.ts/.vue 源文件内容时，永远用 `$content.Replace()` 而不用 `-replace`。**

一旦写坏了，编译报错 `非法字符: '`'` 且源文件中出现裸反引号，必须重写整个文件。

### 新增文件时的反引号规范

用 `@'...'@` Here-String 创建 Java 文件时，不要在内容中使用反引号。Here-String 中的反引号会被 PowerShell 当作转义符处理。
